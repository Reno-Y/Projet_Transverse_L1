...
class Dialogue:
    def __init__(self, screen, height, width, box_width, box_height, x, y, text, color):
        ...
        for lines in self.lines:
            ...
            # Génère une surface pour chaque ligne de texte pour les afficher plus tard sans animation
    ...
    def draw(self):
        if self.run:
            ...
            # Affiche le texte avec une animation de défilement

            for i in range(len(self.lines)):
                if i < self.active_dialogue:
                    self.screen.blit(self.text_surfaces[i], (self.x + 20, self.y + 10 + 40 * i))
                    # Affiche les lignes de texte déjà affichées sans animation
            ...
            # Récupère la ligne de texte correspondant à l'indice actif, puisque le texte est slice par chaque caractère

    def skip(self):
        self.counter += 4
        if self.counter >= len(self.text):
            self.counter = len(self.text)
        # Fait défiler le texte plus rapidement si le joueur appuie sur la touche 'Espace'
    ...
    def next(self):
        ...
        if self.done and self.active_dialogue < len(self.lines) - 1:
            ...
            self.done = False
            self.counter = 0
        # Passe à la ligne de texte suivante si l'animation est terminée et qu'il reste des lignes de texte

    def closed(self):
        if self.active_dialogue == len(self.lines) - 1 and self.done:
            self.run = False
            return True
    # Arrête l'animation si toutes les lignes de texte ont été affichées
