...
Mo_slim_marche_g = Animation(Monde, EClargeur, EChauteur,
                               'Lanceur_projTransverse/Assets/character/MOB/Mob_Slim/Slim_marche_gauche.png', 1.2, 128, 128,
                               (0, 0))
Mo_slim_marche_d = Animation(Monde, EClargeur, EChauteur,
                               'Lanceur_projTransverse/Assets/character/MOB/Mob_Slim/Slim_marche_droite.png', 1.2, 128, 128,
                               (0, 0))
#SEQUENCE_IM_Slim touche_/gauche/droite
Mo_slim_touche_g = Animation(Monde, EClargeur, EChauteur,
                               'Lanceur_projTransverse/Assets/character/MOB/Mob_Slim/Slim_touche_gauche.png', 1.2, 128, 128,
                               (0, 0))
Mo_slim_touche_d = Animation(Monde, EClargeur, EChauteur,
                               'Lanceur_projTransverse/Assets/character/MOB/Mob_Slim/Slim_touche_droite.png', 1.2, 128, 128,
                               (0, 0))
...
class JOUEUR:
    def __init__(self):
        ...
        self.speed = 2
        #Definie les points de vie du joueur
        self.vie = 30000
        self.statut = 0
        #|
        #Definie la portee des attaques
        self.portee = 130
        #Definie les degats causes par les attaques du joueurs, et si celle-ci affligent des status
        self.force = [50, 0]
        #|
        self.scale = 4
    ...
    def regard_statut(self, zonemarquee):
        ...
        for x in range(len(zonemarquee)):
            if self.pos_x > zonemarquee[x][0] and self.pos_x <= zonemarquee[x][2] and self.pos_y > zonemarquee[x][1] and self.pos_y <= zonemarquee[x][3] and self.action[4] == 0:
            ...
            #Si definie, herite un statut au joueur
    ...
    def mouvement_action(self, pos_cam):
        ...
        if ClavIn[pg.K_KP0]:
            #ATTAQUER
            self.action[2] = 1
            zm_g = [self.pos_x - self.portee - 5, self.pos_y - self.portee - 5, self.pos_x - 5, self.pos_y + 50, self.force]
            zm_d = [self.pos_x + 5, self.pos_y - 50, self.pos_x + self.portee + 5, self.pos_y + self.portee + 5, self.force]
...
class MONSTRE:
    def __init__(self, pos_x, pos_y, largeur, hauteur, mass, vitesse, scale, ecran, friction_coef, campeur, attention, etat, vie) -> None:
        # |
    ...
    def PatrouilleRecherche(self, Position_joueur):
        ...
        if self.attention >= abs(self.pos_x) - Position_joueur[0] and self.attention >= abs(self.pos_y) - Position_joueur[0] and self.action[4] != 9:
            self.etat = 1
